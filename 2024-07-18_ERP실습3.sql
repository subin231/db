...
insert into orderitems(orderno,prodno,itemprice,itemdiscount,itemcount)values
...
select c.userName , b.prodName , a.cartProdCount 
  from carts a
  join products b
...
select a.userId, a.userName, a.userHp, a.userPoint, sum(b.`point`) as 총합 
  from users a
  left join points b
...
select a.orderNo, a.userId, b.userName, a.orderTotalPrice, a.orderDate 
  from orders a
  join `users` b
    on a.userId  = b.userId 
 where a.orderTotalPrice >= 100000
 order by a.orderTotalPrice desc , b.userName;
 

#문제5. 모든 주문의 주문번호, 주문자 아이디, 고객명, 상품명, 주문일자를 조회하시오. 주문번호는 중복 없이 상품명은 구분자 ,로 나열할것
select distinct(a.orderNo), a.userId, b.userName, GROUP_CONCAT(d.prodName SEPARATOR ', ') AS 상품명, a.orderDate 
  from orders a
  join `users` b
    on a.userId  = b.userId 
  join orderitems c
...
select prodNo, prodName , prodPrice, prodDiscount, prodPrice-(prodPrice*(1/100*prodDiscount)) as '할인된 가격'
  from products;
...
#그 가격으로 주문별 총합을 구해서 주문별 총합이 10만원이상 그리고 큰 금액 순으로 `주문번호`, `최종총합`을조회하시오.
select orderNo ,sum((itemPrice-(itemPrice*(1/100*itemDiscount)))*itemCount) as `최종총합`
  from orderitems
 group by orderNo
having `최종총합` >= 100000
 order by `최종총합` desc;
...
select b.userName, GROUP_CONCAT(d.prodName SEPARATOR ',  ') as '상품명'
  from orders a
  join `users` b
    on a.userId  = b.userId 
  join orderitems c
